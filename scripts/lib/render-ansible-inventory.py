...
def normalise_group_name(value: str) -> str:
    group = re.sub(r"[^A-Za-z0-9_]+", "_", value.lower()).strip("_")
    if group == "":
        return "service"
    if group[0].isdigit():
        return f"service_{group}"
    return group
...
def collect_proxmox_helper_lxc(values: dict[str, str], ssh_key_path: str) -> list[tuple[str, dict[str, str]]]:
...
def append_proxmox_helper_lxc(lines: list[str], rows: list[tuple[str, dict[str, str]]]) -> None:
    if not rows:
        return
...
def append_service_groups(lines: list[str], rows: list[tuple[str, dict[str, str]]]) -> None:
    service_hosts: dict[str, list[str]] = {}
    for hostname, host_values in rows:
        group = normalise_group_name(host_values.get("homelab_service", ""))
        service_hosts.setdefault(group, []).append(hostname)

    for group in sorted(service_hosts):
        lines.extend([
            f"    {group}:",
            "      hosts:",
        ])
        for hostname in sorted(service_hosts[group]):
            lines.append(f"        {hostname}: {{}}")
...
def render_inventory(values: dict[str, str], ssh_key_path: str) -> str:
    ...
    proxmox_rows = collect_proxmox_helper_lxc(values, ssh_key_path)
    append_proxmox_helper_lxc(lines, proxmox_rows)
    append_service_groups(lines, proxmox_rows)
